...
class Login(ft.View):
    ...
    def login_attempt(self, e):
        ...
        if self.verify_password(self.username_field.value.strip(), self.password_field.value.strip()) is True:
            ...
            # Implementing page.session to store userID - Antony
            # TODO: Ensure all usernames in database are unique
            self.page.session.set("userID", self.user_repo.get_user_id(self.username_field.value.strip())) 
