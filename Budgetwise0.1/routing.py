...
def view_handler(page: ft.Page, db_instance):
    ...
    fpq_instance = ForgotPasswordQuestions(page, user_data)  # Create an instance


    return {
        "/login": Login(page, user_data),
        "/sign_up": SignUp(page, user_data),
        "/security_questions": SecurityQuestions(page, user_data), 
        "/create_budget": CreateBudget(page, user_data),  
        "/dashboard": Dashboard(page, user_data, nav_rail),
        "/username_verification": UsernameVerification(page, user_data, fpq_instance),  # Pass instance
        "/forgot_password_questions": fpq_instance,  # Ensure instance is used here
        "/reset_password": ResetPassword(page, user_data),
        "/reset_password_success": ResetPasswordSuccess(page),
        "/add_budget_accounts": AddBudgetAccounts(page, user_data),
        "/accounts": Accounts(page, user_data, nav_rail),
        "/transactions": Transactions(page, user_data, nav_rail),
        "/history": History(page, user_data, nav_rail),
    }
