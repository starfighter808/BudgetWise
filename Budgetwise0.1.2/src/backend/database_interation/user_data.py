...
### ---- SECURITY QUESTIONS ---- ###
    ...
    def get_budget_accounts_from_db(self):
    ...
    def add_budget_account(self, budget_id, account_name, account_type=0, total_allocated_amount=0.0, current_amount=0.0, savings_goal=0.0, notes=None, importance_rating=0):
        """Add a budget account for the user."""
        try:
            cursor = self.db.cursor()
            cursor.execute(
                """INSERT INTO budget_accounts (user_id, budget_id, account_name, account_type, total_allocated_amount, current_amount, savings_goal, notes, importance_rating)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (self.user_id, budget_id, account_name, account_type, total_allocated_amount, current_amount, savings_goal, notes, importance_rating),
            )
            self.db.commit_db()
            ...
            for account_data in accounts:
                cursor.execute(
                    ...
                    (
                        ...
                        self._map_account_type(account_data.get("type")), # Map dropdown value to integer
                        ...
                        int(account_data.get("importance_rating", 0)) if account_data.get("importance_rating") else 0, # Ensure integer
                    ),
                )
            self.db.commit_db()
    ...
    def _map_account_type(self, account_type_str):
        """Helper function to map dropdown string to the integer value in the DB."""
        if account_type_str == "Checking":
            return 0
        elif account_type_str == "Savings":
            return 1
        elif account_type_str == "Investment":
            return 2
        return 0 # Default to Checking

    def get_budget_accounts(self):
        """Fetch all budget account names for the user (using the updated table)."""
        return self.get_budget_accounts_from_db()

        
    def update_budget(self, name, amount):
        self.cursor.execute(
            "UPDATE budgets SET name = ?, amount = ? WHERE id = ?",
            (name, amount, self.budget_id)
        )
        self.db.commit_db()
