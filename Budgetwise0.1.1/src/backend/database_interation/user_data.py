...
### ---- SECURITY QUESTIONS ---- ###
    ...
    def get_budget_accounts(self):
        ...
        try:
            ...
            self.budget_accounts = [row[0] for row in cursor.fetchall()]
            return self.budget_accounts
        except sqlcipher3.Error as e:
            print(f"Database error while fetching budget accounts: {e}")
            return []

    def add_budget_account(self, account_name, balance):
        """Add a budget account for the user."""
        try:
            cursor = self.db.cursor()
            cursor.execute(
                """INSERT INTO budget_accounts (user_id, account_name, balance)
                VALUES (?, ?, ?)""",
                (self.user_id, account_name, balance),
            )
            self.db.commit_db()
    ...
    def update_budget_account_balance(self, account_name, new_balance):
        """Update the balance of a specific budget account."""
        try:
            cursor = self.db.cursor()
            cursor.execute(
                "UPDATE budget_accounts SET balance = ? WHERE user_id = ? AND account_name = ?",
                (new_balance, self.user_id, account_name),
            )
            self.db.commit_db()
            print(f"Budget account '{account_name}' balance updated to {new_balance}.")
            return True
        except sqlcipher3.Error as e:
            print(f"Database error while updating budget account balance: {e}")
            return False

    def add_budget_accounts(self, accounts):
        """Rewritten function to add multiple budget accounts at once."""
        try:
            cursor = self.db.cursor()
